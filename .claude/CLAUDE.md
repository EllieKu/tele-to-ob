...
- **[bot.mjs](../../bot.mjs)** — 常駐入口。白名單驗證 → 正則抓連結 → 帶重試呼叫 clipCore → 寫 Vault → 回報結果；失敗記錄到 `failed.log`。
...
依 [.env.example](../../.env.example) 建立。必填 `TELEGRAM_BOT_TOKEN`、`OBSIDIAN_VAULT_PATH`；缺任一 bot 直接 exit。`OBSIDIAN_CLIP_FOLDER` 預設 `Clippings`。`TELEGRAM_GROUP_ID` 選填（限定特定群組）。
